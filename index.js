...
app.post('/ajax/email', function (request, response) {
  ...
  const transporter = nodemailer.createTransport({
    ...
    secure: true,
    auth: {
      user: 'hameet.dev@gmail.com', // this should be YOUR GMAIL account
      pass: 'Hze37bh7' // this should be your password
    }
  })

  var textBody = `FROM: ${request.body.name} EMAIL: ${
    request.body.email
  } MESSAGE: ${request.body.message}`
  var htmlBody = `<h2>Mail From Contact Form</h2><p>from: ${
    request.body.name
  } <a href="mailto:${request.body.email}">${request.body.email}</a></p><p>${
  ...
  transporter.sendMail(mail, function (err, info) {
    if (err) {
      ...
      response.json({ message: `Thank you for sending a message` })
